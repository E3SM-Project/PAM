...
void partition_domain(std::string inFile, int &crm_nx, int &crm_ny)
{
  ...
  int py = floor(myrank / nprocx);
  int px = myrank - nprocx * py;

  //Get my beginning and ending global indices; and domain sizes
  double nper = ((double) nx_glob)/nprocx;
  int i_beg = (int) round( nper* px    );
  int i_end = (int) round( nper*(px+1) )-1;
  crm_nx = i_end - i_beg + 1;

  nper = ((double) ny_glob)/nprocy;
  int j_beg = (int) round( nper* py    );
  int j_end = (int) round( nper*(py+1) )-1;
