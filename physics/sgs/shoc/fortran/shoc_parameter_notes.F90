...
! MAIN QUESTIONS:
! * Does SHOC expect ground to be at nlev-1 or zero?
...
subroutine shoc_main ( &
  ...
  integer    , intent(in   ) :: nadv                                   !QUESTION     ARE THESE PAIRED? E.G., HALVE THE TIME STEP WITH TWO LOOPS? number of times to loop SHOC
  real(rtype), intent(in   ) :: dtime                                  !QUESTION     ARE THESE PAIRED? E.G., HALVE THE TIME STEP WITH TWO LOOPS? SHOC timestep [s]
  ...
  real(rtype), intent(in   ) :: thv         (shcol,nlev              ) !QUESTION     MAKE SURE OF DEFINITION virtual potential temperature [K]
  real(rtype), intent(in   ) :: w_field     (shcol,nlev              ) !QUESTION     WHY "FIELD" INSTEAD OF "WIND"?  large scale vertical velocity [m/s]
  ...
  real(rtype), intent(inout) :: thetal      (shcol,nlev              ) !QUESTION     https://glossary.ametsoc.org/wiki/Liquid_water_potential_temperature ??  liquid water potential temperature [K]
  ...
  real(rtype), intent(inout) :: wthv_sec    (shcol,nlev              ) !             buoyancy flux [K m/s]
  real(rtype), intent(inout) :: qtracers    (shcol,nlev ,num_qtracers) !QUESTION     MASS OR DRY MIXING RATIOS? tracers [varies]
  real(rtype), intent(inout) :: tk          (shcol,nlev              ) !             eddy coefficient for momentum [m2/s]
  real(rtype), intent(inout) :: tkh         (shcol,nlev              ) !             eddy coefficent for heat [m2/s]
  real(rtype), intent(inout) :: shoc_cldfrac(shcol,nlev              ) !             Cloud fraction [-]
