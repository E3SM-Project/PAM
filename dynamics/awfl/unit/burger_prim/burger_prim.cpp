...
void print_states( realHost1d const &state1 , realHost1d const &state2 ) {
  for (int i=0; i < state1.dimension[0]; i++) {
    std::cout << std::scientific << state1(i) << "  " << state2(i) << "\n";
  }
}



void print_states( realHost1d const &state1 , realHost1d const &state2 , realHost1d const &state3 ) {
  for (int i=0; i < state1.dimension[0]; i++) {
    std::cout << std::scientific << state1(i) << "  " << state2(i) << "  " << state3(i) << "\n";
  }
}
...
template <int ord>
realHost1d simulate_cons( int nx , real sim_time , bool weno ) {
...
template <int ord>
realHost1d simulate_prim1( int nx , real sim_time , bool weno ) {
  int constexpr hs = (ord-1)/2;
  ...
  while (etime < sim_time) {
    ...
    for (int i=0; i < nx; i++) {
      ...
      // Reconstruct
      SArray<real,1,ord> u ;
      SArray<real,1,ord> du;
      if (weno) {
        SArray<real,1,ord> wenoCoefs;
        weno::compute_weno_coefs<ord>( wenoRecon , stencil , wenoCoefs , idl , sigma );
        u  = c2g * wenoCoefs;
        du = c2g * c2d * wenoCoefs;
      } else {
        u  = c2g * s2c * stencil;
        du = c2g * c2d * s2c * stencil;
      }
...
template <int ord>
realHost1d simulate_prim2( int nx , real sim_time , bool weno ) {
  int constexpr hs = (ord-1)/2;
  ...
  while (etime < sim_time) {
    ...
    for (int i=0; i < nx; i++) {
      ...
      ctend (1,i  ) = -u_du_DTs(0,0    ) * gllWts_ord(0    );
      ctend (0,i+1) = -u_du_DTs(0,ord-1) * gllWts_ord(ord-1);
    }
    ...
    for (int i=0; i < nx+1; i++) {
      ...
      if (i > 0 ) tend(i-1) += ctend(0,i);
      if (i < nx) tend(i  ) += ctend(0,i);
    }
...
int main() {
  real constexpr sim_time = 0.001;
  bool constexpr weno = false;

  // realHost1d state_hi = simulate_cons<5>( 1000 , sim_time );

  // real l1_100 = L1( simulate_prim1<5>( 100 , sim_time ) , state_hi );
  // real l1_50  = L1( simulate_prim1<5>( 50  , sim_time ) , state_hi );

  // std::cout << log(l1_100 / l1_50) / log(50./100.) << "\n";
  // // print_state( simulate_prim1<5>( 100 , sim_time ) );

  print_states( simulate_cons<9>(100,sim_time,weno) , simulate_prim1<9>(100,sim_time,weno) , simulate_prim2<9>(100,sim_time,weno) );
}
