...
public:
  ...
  // TODO: I need to figure out how to compute DTs for rho*w over the course of the larger advection time step
  //       (1) Averate DTs over acoustic time steps;  (2) Save tord time steps including t_n and t_{n+1}; compute DTs
  // TODO: How to handle Riemann solver.
  //       (1) Can we acoustically upwind rho*w over larger advective time step and still maintain stability?
  //       (2) Can we get away with only advectively upwinding everything except rho*w and rho*theta?
  // TODO: I can't imagine a scenario where we do not have to recompute rho based on a new rho*w for mass consistency
  //       Likely, we can just divide rho*w and rho*theta by old density and multiply by new since strict conservation
  //          isn't important for those two
  void computeTendenciesZ( real5d const &state   , real5d const &stateTend  ,
    ...
    // Compute the number of subcycles needed for acousts and the subcycled time step
    real4d dt4d("dt4d",nz,ny,nx,nens);
    parllel_for( Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
      ...
      real cs = sqrt( gamm * p / r );
      dt4d(k,j,i,iens) = cfl * dz(k,iens) / max( abs(w-cs) , abs(w+cs) );
    });
    real dt_acoust = yakl::intrinsics::minval( dt4d );
    // We need "tord" differential transforms in time, so we need at least tord-1 points in time to create them
    int num_subcycles = (int) std::ceil( std::max( tord-1._fp , dt / dt_acoust ) );
    if (tord%2 == 1 && num_subcycles%2 == 0) num_subcycles++;
    dt_acoust = dt / num_subcycles;

    SArray<int,1,tord> time_steps_save;
    time_steps_save(0) = 0;
    time_steps_save(tord-1) = num_subcycles-1;

    real5d state_save = state.createDeviceCopy();

    for (int icycle = 0; icycle < num_subdycles; icycle++) {

      int constexpr loc_idR=0;
      int constexpr loc_idW=1;
      int constexpr loc_idT=2;

      int constexpr loc_num_state = 3;

      // Stores two estimates of the state and tracer values at each cell interface
      real6d stateLimits("stateLimits",loc_num_state,2,nz+1,ny,nx,nens);
      real4d grav_source("grav_source",nz,ny,nx,nens);
      ...
      parallel_for( Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
        ...
        SArray<real,2,nAder,ngll> r_DTs , rw_DTs , rt_DTs, rww_DTs, rt_gamma_DTs, rwt_DTs;
        ...
        diffTransformEulerZ_subcycle( r_DTs , rw_DTs , rt_DTs , rww_DTs , rwt_DTs , rt_gamma_DTs ,
                                      derivMatrix , hyPressureGLL , C0 , gamma , k , dz(k,iens) , bc_z , nz , iens );
        ...
        //////////////////////////////////////////
        // Left interface
        stateLimits(loc_idR,1,k  ,j,i,iens) = r_DTs (0,0     );
        stateLimits(loc_idW,1,k  ,j,i,iens) = rw_DTs(0,0     );
        stateLimits(loc_idT,1,k  ,j,i,iens) = rt_DTs(0,0     );
        // Right interface
        stateLimits(loc_idR,0,k+1,j,i,iens) = r_DTs (0,ngll-1);
        stateLimits(loc_idW,0,k+1,j,i,iens) = rw_DTs(0,ngll-1);
        stateLimits(loc_idT,0,k+1,j,i,iens) = rt_DTs(0,ngll-1);
      ...
      parallel_for( "Spatial.h Z Riemann" , SimpleBounds<4>(nz+1,ny,nx,nens) ,
                    YAKL_LAMBDA (int k, int j, int i, int iens) {
        // Get left and right state
        real r_L = stateLimits(loc_idR,0,k,j,i,iens)    ;   real r_R = stateLimits(loc_idR,1,k,j,i,iens)    ;
        real w_L = stateLimits(loc_idW,0,k,j,i,iens)/r_L;   real w_R = stateLimits(loc_idW,1,k,j,i,iens)/r_R;
        real t_L = stateLimits(loc_idT,0,k,j,i,iens)/r_L;   real t_R = stateLimits(loc_idT,1,k,j,i,iens)/r_R;
        // Compute average state
        real r = 0.5_fp * (r_L + r_R);
        real w = 0.5_fp * (w_L + w_R);
        real t = 0.5_fp * (t_L + t_R);
        ...
        real cs  = sqrt(cs2);
        // Get left and right fluxes
        real q1_L = stateLimits(loc_idR,0,k,j,i,iens);   real q1_R = stateLimits(loc_idR,1,k,j,i,iens);
        real q4_L = stateLimits(loc_idW,0,k,j,i,iens);   real q4_R = stateLimits(loc_idW,1,k,j,i,iens);
        real q5_L = stateLimits(loc_idT,0,k,j,i,iens);   real q5_R = stateLimits(loc_idT,1,k,j,i,iens);
        // Compute upwind characteristics
        // w
        real w1;
        if (w > 0) {
        ...
        stateLimits(loc_idR,0,k,j,i,iens) = q4;
        stateLimits(loc_idW,0,k,j,i,iens) = q4*q4/q1 + C0*pow(q5,gamma);
        stateLimits(loc_idT,0,k,j,i,iens) = q4*q5/q1;
      });

      parallel_for( SimpleBounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA(int k, int j, int i, int iens) {
        real rdz = 1._fp / dz(k,iens);
        state(idR,k,j,i,iens) += - dt_acoust*( stateLimits(loc_idR,0,k+1,j,i,iens) - stateLimits(loc_idR,0,k,j,i,iens) )*rdz;
        state(idW,k,j,i,iens) += - dt_acoust*( stateLimits(loc_idW,0,k+1,j,i,iens) - stateLimits(loc_idW,0,k,j,i,iens) )*rdz;
        state(idT,k,j,i,iens) += - dt_acoust*( stateLimits(loc_idT,0,k+1,j,i,iens) - stateLimits(loc_idT,0,k,j,i,iens) )*rdz;
        state(idW,k,j,i,iens) += dt_acoust * grav_source(k,j,i,iens);
      });
    }
  ...
  YAKL_INLINE static void diffTransformEulerZ_subcycle( SArray<real,2,nAder,ngll> &r  ,
                                                        SArray<real,2,nAder,ngll> &rw ,
                                                        SArray<real,2,nAder,ngll> &rt ,
                                                        SArray<real,2,nAder,ngll> &rww ,
                                                        SArray<real,2,nAder,ngll> &rwt ,
