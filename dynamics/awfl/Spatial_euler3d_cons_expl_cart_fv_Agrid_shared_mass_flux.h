...
public:
  ...
  void computeTendenciesX( StateArr  &state   , StateTendArr  &stateTend  ,
    ...
    parallel_for( Bounds<3>(nz,ny,nx) , YAKL_LAMBDA (int k, int j, int i) {
      ...
      { // BEGIN: Reconstruct, time-average, and store tracer fluxes
        ...
        for (int tr=0; tr < numTracers; tr++) {
          ...
          ////////////////////////////////////////////////////////////
          tracerLimits    (tr,1,k,j,i  ) = rt_DTs (0,0     ) / r_DTs(0,0     ); // Left interface
          tracerLimits    (tr,0,k,j,i+1) = rt_DTs (0,ngll-1) / r_DTs(0,ngll-1); // Right interface
    ...
    parallel_for( Bounds<3>(nz,ny,nx+1) , YAKL_LAMBDA (int k, int j, int i) {
      ...
      for (int tr=0; tr < numTracers; tr++) {
        if (u > 0) {
          tracerFluxLimits(tr,0,k,j,i) = stateFluxLimits(idR,0,k,j,i) * tracerLimits(tr,0,k,j,i);
        } else {
          tracerFluxLimits(tr,0,k,j,i) = stateFluxLimits(idR,0,k,j,i) * tracerLimits(tr,1,k,j,i);
        }
      }
    });
    ...
    parallel_for( Bounds<3>(nz,ny,nx) , YAKL_LAMBDA (int k, int j, int i) {
      ...
      { // BEGIN: Reconstruct, time-average, and store tracer fluxes
        ...
        for (int tr=0; tr < numTracers; tr++) {
          ...
          tracerFluxLimits(tr,1,k,j  ,i) = rvt_DTs(0,0     ); // Left interface
          tracerFluxLimits(tr,0,k,j+1,i) = rvt_DTs(0,ngll-1); // Right interface
        }
      } // END: Reconstruct, time-average, and store tracer fluxes
    ...
    parallel_for( Bounds<3>(nz,ny,nx) , YAKL_LAMBDA (int k, int j, int i) {
      ...
      { // BEGIN: Reconstruct, time-average, and store tracer fluxes
        ...
        for (int tr=0; tr < numTracers; tr++) {
          ...
          ////////////////////////////////////////////////////////////
          tracerLimits    (tr,1,k  ,j,i) = rt_DTs (0,0     ) / r_DTs (0,0     ); // Left interface
          tracerLimits    (tr,0,k+1,j,i) = rt_DTs (0,ngll-1) / r_DTs (0,ngll-1); // Right interface
