...
public:
  ...
  void remove_momentum_divergence(real5d &state) {
    ...
    // ADI version
    ////////////////////////////////////
    // Build the iteration matrices once TODO: move outside this routine and save
    Eigen::SparseMatrix<real> A_x(2*nx,2*nx);
    A_x.reserve(Eigen::VectorXi::Constant(2*nx,6)); 

    // Interior pressure coeffs
    for( int i=2; i < 2*nx-3; i+=2 ) {
       A_x.insert(i,i-2) = -xi_x/2.;
       A_x.insert(i,i-1) = -xi_x/2.*c_s;
       A_x.insert(i,i  ) = 1.+xi_x;
       //A_x.insert(i,i+1) = 0.; // zero placeholder, doesn't actually set
       A_x.insert(i,i+2) = -xi_x/2.;
       A_x.insert(i,i+3) = xi_x/2.*c_s;
    }
    // Interior momentum coeffs
    for( int i=3; i < 2*nx-2; i+=2 ) {
       A_x.insert(i,i-3) = -xi_x/2./c_s;
       A_x.insert(i,i-2) = -xi_x/2.;
       //A_x.insert(i,i-1) = 0.;
       A_x.insert(i,i  ) = 1+xi_x;
    ...
    // Pressure BC coeffs
    A_x.insert(0,2*nx-2) = -xi_x/2.;
    A_x.insert(0,2*nx-1) = -xi_x/2.*c_s;
    A_x.insert(0,0) = 1.+xi_x;
    A_x.insert(0,2) = -xi_x/2.;
    A_x.insert(0,3) = xi_x/2.*c_s;
    A_x.insert(2*nx-2,2*nx-4) = -xi_x/2.;
    A_x.insert(2*nx-2,2*nx-3) = -xi_x/2.*c_s;
    A_x.insert(2*nx-2,2*nx-2) = 1.+xi_x;
    A_x.insert(2*nx-2,0) = -xi_x/2.;
    A_x.insert(2*nx-2,1) = xi_x/2.*c_s;

    // Momentum BC coeffs
    A_x.insert(1,2*nx-2) = -xi_x/2./c_s;
    A_x.insert(1,2*nx-1) = -xi_x/2.;
    A_x.insert(1,1) = 1+xi_x;
    A_x.insert(1,2) = xi_x/2./c_s;
    A_x.insert(1,3) = -xi_x/2.;
    A_x.insert(2*nx-1,2*nx-4) = -xi_x/2./c_s;
    A_x.insert(2*nx-1,2*nx-3) = -xi_x/2.;
    A_x.insert(2*nx-1,2*nx-1) = 1+xi_x;
    A_x.insert(2*nx-1,0) = xi_x/2./c_s;
    A_x.insert(2*nx-1,1) = -xi_x/2.;

    A_x.makeCompressed();

    Eigen::SparseMatrix<real> A_z(2*nz,2*nz);
    A_z.reserve(Eigen::VectorXi::Constant(2*nz,6));

    // Interior pressure coeffs
    for( int k=2; k < 2*nz-3; k+=2 ) {
       A_z.insert(k,k-2) = -xi_z/2.;
       A_z.insert(k,k-1) = -xi_z/2.*c_s;
       A_z.insert(k,k  ) = 1.+xi_z;
       //A_z.insert(k,k+1) = 0.;
       A_z.insert(k,k+2) = -xi_z/2.;
       A_z.insert(k,k+3) = xi_z/2.*c_s;
    }
    // Interior momentum coeffs
    for( int k=3; k < 2*nz-2; k+=2 ) {
       A_z.insert(k,k-3) = -xi_z/2./c_s;
       A_z.insert(k,k-2) = -xi_z/2.;
       //A_z.insert(k,k-1) = 0.;
       A_z.insert(k,k  ) = 1+xi_z;
    ...
    // Pressure BC coeffs
    A_z.insert(0,0) = 1.+xi_z/2.;
    A_z.insert(0,1) = xi_z/2.*c_s;
    A_z.insert(0,2) = -xi_z/2.;
    A_z.insert(0,3) = xi_z/2.*c_s;
    A_z.insert(2*nz-2,2*nz-4) = -xi_z/2.;
    A_z.insert(2*nz-2,2*nz-3) = -xi_z/2.*c_s;
    A_z.insert(2*nz-2,2*nz-2) = 1.+xi_z/2.;
    A_z.insert(2*nz-2,2*nz-1) = -xi_z/2.*c_s;

    // Momentum BC coeffs
    A_z.insert(1,0) = -xi_z/2./c_s;
    A_z.insert(1,1) = 1.+xi_z/2.;
    A_z.insert(1,2) = xi_z/2./c_s;
    A_z.insert(1,3) = -xi_z/2.;
    A_z.insert(2*nz-1,2*nz-4) = -xi_z/2./c_s;
    A_z.insert(2*nz-1,2*nz-3) = -xi_z/2.;
    A_z.insert(2*nz-1,2*nz-2) = xi_z/2./c_s;
    A_z.insert(2*nz-1,2*nz-1) = 1.+xi_z/2.;

    A_z.makeCompressed();

    Eigen::BiCGSTAB<Eigen::SparseMatrix<real> > solver_x;
    solver_x.compute(A_x);
    Eigen::BiCGSTAB<Eigen::SparseMatrix<real> > solver_z;
    solver_z.compute(A_z);

    for (int iter=0; iter < 50; iter++) {

       // Fill RHS vecto, perform x-direction implicit solve, store result back in state arrays
       // stacking these in YAKL parallel for threw error: call to implicitly-deleted copy constructor
       for( int k=0; k < nz; k++ ) {
          for( int j=0; j < ny; j++ ) {
             for( int iens=0; iens < nens; iens++ ) {
                Eigen::VectorXd q(2*nx), b(2*nx), q0(2*nx);

                for( int i=0; i < nx; i++ ) {
                   b(2*i) = pressure(k,j,i,iens);
                   b(2*i+1) = state(idU,hs+k,hs+j,hs+i,iens) * hyDensCells(hs+k,iens);
                   q0(2*i) = pressure(k,j,i,iens);
                   q0(2*i+1) = rho_u_new(k,j,i,iens);
                }
                q = solver_x.solveWithGuess(b,q0);
                for( int i=0; i < nx; i++ ) {
                   pressure(k,j,i,iens) = q(2*i);
                   rho_u_new(k,j,i,iens) = q(2*i+1);
                }
             }
          }
       }

      // Fill RHS vector and perform z-direction implicit solve
       for( int j=0; j < ny; j++ ) {
          for( int i=0; i < nx; i++ ) {
             for( int iens=0; iens < nens; iens++ ) {
                Eigen::VectorXd q(2*nz), b(2*nz), q0(2*nz);

                for( int k=0; k < nz; k++ ) {
                   b(2*k) = pressure(k,j,i,iens);
                   b(2*k+1) = state(idW,hs+k,hs+j,hs+i,iens) * hyDensCells(hs+k,iens);
                   q0(2*k) = pressure(k,j,i,iens);
                   q0(2*k+1) = rho_w_new(k,j,i,iens);
                }
                q = solver_z.solveWithGuess(b,q0);
                for( int k=0; k < nz; k++ ) {
                   pressure(k,j,i,iens) = q(2*k);
                   rho_w_new(k,j,i,iens) = q(2*k+1);
                }
             }
          }
       }
