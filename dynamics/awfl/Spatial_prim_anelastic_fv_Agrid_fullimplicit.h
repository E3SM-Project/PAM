...
public:
  ...
  void remove_momentum_divergence(real5d &state) {
    ...
    /////////////////////////////////////////////
    // Full Implicit / Backward Euler version
    /////////////////////////////////////////////
    // Build the dimensionally-coupled iteration matrix once TODO: move outside this routine and save
    Eigen::SparseMatrix<real> A(3*nx*nz,3*nx*nz);
    A.reserve(Eigen::VectorXi::Constant(3*nx*nz,6)); 

    // Interior cell coefficients
    for( int i=0; i < nx; i++ ){
       for( int k=1; k < nz-1; k++ ) {
          int pik, uik, wik, pim1, uim1, pip1, uip1, pkm1, wkm1, pkp1, wkp1;
          pik  = 3*( k*nx+i); uik  = pik+1;  wik = uik+1;
    ...
    for( int i=0; i < nx; i++ ){
       ...
       A.insert(pik,pim1) = -xi_x/2.;
       A.insert(pik,uim1) = -xi_x/2.*c_s;
       A.insert(pik,pik) = 1.+xi_x+xi_z/2.;
       A.insert(pik,pip1) = -xi_x/2.;
    ...
    for (int iter=0; iter < 50; iter++) {
       ...
       // Fill RHS vector and perform x-direction implicit solve
       // stacking these in YAKL parallel for threw error: call to implicitly-deleted copy constructor
       Eigen::VectorXd q(3*nx*nz), b(3*nx*nz), q0(3*nx*nz);
       for( int k=0; k < nz; k++ ) {
          for( int j=0; j < ny; j++ ) {
             for( int i=0; i < nx; i++ ){
                for( int iens=0; iens < nens; iens++ ) {
                   ...
                   b(pik) = pressure(k,j,i,iens);
                   b(uik) = state(idU,hs+k,hs+j,hs+i,iens) * hyDensCells(hs+k,iens);
                   b(wik) = state(idW,hs+k,hs+j,hs+i,iens) * hyDensCells(hs+k,iens);

                   q0(pik) = pressure(k,j,i,iens);
                   q0(uik) = rho_u_new(k,j,i,iens);
                   q0(wik) = rho_w_new(k,j,i,iens);
                }
             }
          }
       }
       q = solver.solveWithGuess(b,q0);
       for( int k=0; k < nz; k++ ) {
          for( int j=0; j < ny; j++ ) {
             for( int i=0; i < nx; i++ ){
                for( int iens=0; iens < nens; iens++ ) {
