...
namespace awfl {
  ...
  void riemann_rho_theta_full_z(pam::PamCoupler const &coupler, real6d const & state_limits  , real5d const &state_flux  ,
    ...
    parallel_for( "Spatial.h Z Riemann" , SimpleBounds<4>(nz+1,ny,nx,nens) ,
                  YAKL_LAMBDA (int k, int j, int i, int iens) {
      if (k == 0) {
        for (int l = 0; l < num_state  ; l++) { state_limits (l,0,0 ,j,i,iens) = state_limits (l,1,0 ,j,i,iens); }
        for (int l = 0; l < num_tracers; l++) { tracer_limits(l,0,0 ,j,i,iens) = tracer_limits(l,1,0 ,j,i,iens); }
      }
      if (k == nz) {
        for (int l = 0; l < num_state  ; l++) { state_limits (l,1,nz,j,i,iens) = state_limits (l,0,nz,j,i,iens); }
        for (int l = 0; l < num_tracers; l++) { tracer_limits(l,1,nz,j,i,iens) = tracer_limits(l,0,nz,j,i,iens); }
      }
      // Get left and right state
      real r_L = state_limits(idR,0,k,j,i,iens)    ;   real r_R = state_limits(idR,1,k,j,i,iens)    ;
      real u_L = state_limits(idU,0,k,j,i,iens)/r_L;   real u_R = state_limits(idU,1,k,j,i,iens)/r_R;
      ...
      for (int tr=0; tr < num_tracers; tr++) {
        if (w > 0) {
          tracer_flux(tr,k,j,i,iens) = q4 * tracer_limits(tr,0,k,j,i,iens) / r_L;
        } else {
          tracer_flux(tr,k,j,i,iens) = q4 * tracer_limits(tr,1,k,j,i,iens) / r_R;
        }
      }
    });
  }
