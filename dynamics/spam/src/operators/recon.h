...
void YAKL_INLINE compute_twisted_recon(const real5d &reconvar,
  ...
  compute_Hext<1, diff_ord>(uvar, V, pgeom, dgeom, is, js, ks, i, j, k,
                              n);

  for (int d = 0; d < ndims; d++) {
    //uvar(d) = V(d, k + ks, j + js, i + is, n);
    for (int l = 0; l < ndofs; l++) {
      if (d == 0) {
...
void YAKL_INLINE compute_twisted_vert_recon(const real5d &vertreconvar,
  ...
  //uvar(0) = W(0, k + ks, j + js, i + is, n);
  compute_Hv<1, vert_diff_ord>(uvar, W, pgeom,
                               dgeom, is, js, ks, i, j, k, n);
...
void YAKL_INLINE compute_straight_xz_recon(const real5d &reconvar,
  ...
  //uvar(0) = V(0, k + ks, j + js, i + is, n);
  
  SArray<real, 1, 1> f0, f1, f2, f3;
  compute_Hext<1, diff_ord>(f0, V, pgeom,
                                  dgeom, is, js, ks, i, j, k,
                                  n);
  compute_Hext<1, diff_ord>(f1, V, pgeom,
                                   dgeom, is, js, ks, i + 1, j, k,
                                  n);
  compute_Hext<1, diff_ord>(f2, V, pgeom,
                                   dgeom, is, js, ks, i, j, k + 1,
                                  n);
  compute_Hext<1, diff_ord>(f3, V, pgeom,
                                   dgeom, is, js, ks, i + 1, j, k + 1,
                                  n);
...
void YAKL_INLINE compute_straight_xz_vert_recon(const real5d &reconvar,
  ...
  SArray<real, 1, 1> f0, f1, f2, f3;
  compute_Hv<1, vert_diff_ord>(f0, W, pgeom,
                                  dgeom, is, js, ks, i, j, k,
                                  n);
  compute_Hv<1, vert_diff_ord>(f1, W, pgeom,
                                   dgeom, is, js, ks, i - 1, j, k,
                                  n);
  compute_Hv<1, vert_diff_ord>(f2, W, pgeom,
                                   dgeom, is, js, ks, i, j, k + 1,
                                  n);
  compute_Hv<1, vert_diff_ord>(f3, W, pgeom,
                                   dgeom, is, js, ks, i - 1, j, k + 1,
                                  n);
  SArray<real, 1, 4> flux;
  flux(0) = f0(0);
