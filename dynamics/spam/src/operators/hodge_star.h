...
real YAKL_INLINE compute_H1vert(const real5d &wvar,
                                const Geometry<Straight> &pgeom,
                                const Geometry<Twisted> &dgeom, int is, int js,
                                int ks, int i, int j, int k, int n) {
...
void YAKL_INLINE compute_H2bar(SArray<real, 1, ndofs> &x0, const real5d &var,
  ...
  const real H2bargeom = pgeom.get_area_lform(0, 0, k + ks, j + js, i + is) /
...
void YAKL_INLINE compute_H2barext(F f, SArray<real, 1, ndofs> &x0,
                                  const real5d &var,
                                  const Geometry<Straight> &pgeom,
                                  const Geometry<Twisted> &dgeom, int is,
                                  int js, int ks, int i, int j, int k, int n) {
  SArray<real, 3, ndofs, ndims, hord - 1> x;
  const real H2bargeom = pgeom.get_area_00entity(k + ks, j + js, i + is) /
...
template <uint ndofs, uint hord, uint vord, uint hoff = hord / 2 - 1,
          uint voff = vord / 2 - 1>
void YAKL_INLINE compute_H2barext(SArray<real, 1, ndofs> &x0, const real5d &var,
                                  const Geometry<Straight> &pgeom,
                                  const Geometry<Twisted> &dgeom, int is,
                                  int js, int ks, int i, int j, int k, int n) {
  compute_H2barext<ndofs, hord, vord>(nullptr, x0, var, pgeom, dgeom, is, js,
                                      ks, i, j, k, n);
}
...
template <uint ndofs, uint hord, uint vord,
          ...
          uint voff = vord / 2 - 1, class F>
void YAKL_INLINE compute_H2barext(F f, const real5d &var0, const real5d &var,
                                  const Geometry<Straight> &pgeom,
                                  const Geometry<Twisted> &dgeom, int is,
                                  int js, int ks, int i, int j, int k, int n) {
  SArray<real, 1, ndofs> x0;
  compute_H2barext<ndofs, hord, vord, hoff, voff>(f, x0, var, pgeom, dgeom, is,
                                                  js, ks, i, j, k, n);
  for (int l = 0; l < ndofs; l++) {
    if (addmode == ADD_MODE::REPLACE) {
      var0(l, k + ks, j + js, i + is, n) = x0(l);
...
template <uint ndofs, uint hord, uint vord,
          ADD_MODE addmode = ADD_MODE::REPLACE, uint hoff = hord / 2 - 1,
          uint voff = vord / 2 - 1>
void YAKL_INLINE compute_H2barext(const real5d &var0, const real5d &var,
                                  const Geometry<Straight> &pgeom,
                                  const Geometry<Twisted> &dgeom, int is,
                                  int js, int ks, int i, int j, int k, int n) {
  compute_H2barext<ndofs, hord, vord, addmode, hoff, voff>(
      nullptr, var0, var, pgeom, dgeom, is, js, ks, i, j, k, n);
}

template <uint ord, int off = ord / 2 - 1>
real YAKL_INLINE fourier_H2barext(const Geometry<Straight> &pgeom,
                                  const Geometry<Twisted> &dgeom, int is,
                                  int js, int ks, int i, int j, int k, int n,
  ...
  const real H2bargeom =
      pgeom.get_area_00entity(k + ks, j + js, i + is + 0 - off) /
