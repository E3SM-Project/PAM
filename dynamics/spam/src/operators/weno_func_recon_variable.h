...
YAKL_INLINE void map_weights2(SArray<real, 1, (ord - 1) / 2 + 2> const &idl,
  ...
  for (int i = 0; i < hs + 2; i++) {
    wts(i) =
        wts(i) *
        (idl(i) + idl(i) * idl(i) - 3._fp * idl(i) * wts(i) + wts(i) * wts(i)) /
...
compute_weno_coefs2(SArray<real, 3, (ord - 1) / 2 + 1, (ord - 1) / 2 + 1,
  ...
  // Compute three quadratic polynomials (left, center, and right) and the
  ...
  // WENO weights are proportional to the inverse of TV**2 and then
  // re-confexified
  for (int i = 0; i < hs + 2; i++) {
  ...
  // WENO polynomial is the weighted sum of candidate polynomials using WENO
  // weights instead of ideal weights
  for (int i = 0; i < ord; i++) {
...
weno_func_vert(SArray<real, 2, ndofs, 2> &edgerecon,
  ...
  for (int l = 0; l < ndofs; l++) {
    ...
    // Reconstruct values
    compute_weno_coefs2<ord>(weno_recon_lower, sten_to_coefs, stencil,
                             wenoCoefs, wenoIdl, wenoSigma);
...
void create_variable_WENO(real4d coefs_to_gll_arr, real4d sten_to_gll_arr,
  ...
  parallel_for(
      "Compute Vertically Variable WENO Func Arrays",
      SimpleBounds<2>(topo.nl, topo.nens), YAKL_LAMBDA(int k, int n) {
        ...
        TransformMatrices_variable::sten_to_coefs_variable<ord>(locs,
                                                                s2c_var_in);
        TransformMatrices_variable::weno_lower_sten_to_coefs<ord>(
            locs, weno_recon_lower);
        ...
        for (int h1 = 0; h1 < hs + 1; h1++) {
          for (int h2 = 0; h2 < hs + 1; h2++) {
            for (int h3 = 0; h3 < hs + 1; h3++) {
              weno_recon_lower_arr(k, h1, h2, h3, n) =
                  weno_recon_lower(h1, h2, h3);
            }
          }
        }
      });
