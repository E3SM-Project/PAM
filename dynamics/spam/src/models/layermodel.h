...
public:
 ...
 void compute_diag(const FieldSet<nconstant> &const_vars, FieldSet<nprognostic> &x, FieldSet<ndiagnostic> &diagnostic_vars)
 {
  ...
  int pks = primal_topology->ks;
    ...
    parallel_for("Compute Dens0 Diag", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  compute_I<ndensity, diff_ord>(diagnostic_vars.fields_arr[DENSLDIAGVAR].data, x.fields_arr[DENSVAR].data, *this->primal_geometry, *this->dual_geometry, pis, pjs, pks, i, j, k, n);
    ...
    parallel_for("Compute Q0 Diag", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
...
{varset.convert_coupler_to_dynamics_state(coupler, prog_vars, const_vars);}
  ...
  // compute dens0var = I densvar
          parallel_for("Compute Dens0", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
          compute_I<ndensity, diff_ord>(dens0var, densvar, *this->primal_geometry, *this->dual_geometry, pis, pjs, pks, i, j, k, n);
  });

  // compute U = H v, q0, f0
    parallel_for("Compute U, Q0, F0", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
      ...
      void  YAKL_INLINE compute_functional_derivatives_and_diagnostic_quantities_II(
        real5d Fvar, real5d Kvar, real5d HEvar, const real5d Vvar, const real5d Uvar, const real5d dens0var) {
          ...
          int dks = dual_topology->ks;
            ...
            parallel_for("Compute F/K/HE", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  ...
  int pks = primal_topology->ks;
          ...
          parallel_for("Compute FT/B", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  ...
  Hk.compute_dKddens(Bvar, Kvar, pis, pjs, pks, i, j, k, n);
    ...
    void YAKL_INLINE compute_edge_reconstructions(real5d densedgereconvar, real5d Qedgereconvar, real5d fedgereconvar,
      const real5d dens0var, const real5d Q0var, const real5d f0var) {
        ...
        parallel_for("Compute straight edge recons", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
        ...
        parallel_for("Compute twisted edge recons", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
    ...
    const real5d FTvar, const real5d Uvar) {
      ...
      int dks = dual_topology->ks;
        ...
        parallel_for("Compute straight recons", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
        ...
        parallel_for("Compute twisted recons", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
    ...
    const real5d Bvar, const real5d Fvar, const real5d Phivar) {
      ...
      int dks = dual_topology->ks;
          ...
          parallel_for("Compute v tend", SimpleBounds<4>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x, primal_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  ...
  int dks = dual_topology->ks;
    ...
    parallel_for("Compute edgefluxes", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  ...
  parallel_for("Compute Mf", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
  ...
  parallel_for("Compute Phi", SimpleBounds<4>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x, dual_topology->nens) , YAKL_LAMBDA(int k, int j, int i, int n) { 
...
int dks = dual_topology->ks;
      ...
      parallel_for("Compute energy stats", SimpleBounds<3>( dual_topology->nl, dual_topology->n_cells_y, dual_topology->n_cells_x) , YAKL_LAMBDA(int k, int j, int i) { 
         real KE, PE, IE;
...
int pks = primal_topology->ks;
  ...
  parallel_for("Compute pv/pens stats", SimpleBounds<3>( primal_topology->nl, primal_topology->n_cells_y, primal_topology->n_cells_x) , YAKL_LAMBDA(int k, int j, int i) { 
   pvpe vals_pvpe;
