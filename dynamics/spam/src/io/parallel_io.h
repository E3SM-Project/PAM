...
public:
  ...
  template<uint nprog, uint nconst, uint ndiag, uint nstats> void FileIO<nprog,nconst,ndiag,nstats>::initialize(std::string outName, Topology &ptopo, Topology &dtopo, Parallel &par, const FieldSet<nprog> &progvars, const FieldSet<nconst> &constvars, const FieldSet<ndiag> &diagvars, Stats<nprog, nconst, nstats> &stats)
  {
       ...
       ncwrap( ncmpi_def_dim( ncid , "nens" , (MPI_Offset) ptopo.nens  , &nensDim ) , __LINE__ );
       ...
       for (int i=0; i<this->const_vars->fields_arr.size(); i++)
       {
         ...
         if (this->const_vars->fields_arr[i].topology->primal) { 
           ...
           {const_dim_ids[0] = const_var_dim_ids[i]; const_dim_ids[1] = plDim; const_dim_ids[2] = pyDim; const_dim_ids[3] = pxDim; const_dim_ids[4] = nensDim;}
           if (this->const_vars->fields_arr[i].extdof == 0)
           {const_dim_ids[0] = const_var_dim_ids[i]; const_dim_ids[1] = piDim; const_dim_ids[2] = pyDim; const_dim_ids[3] = pxDim; const_dim_ids[4] = nensDim;}
         }
         else {
           if (this->const_vars->fields_arr[i].extdof == 1)
           {const_dim_ids[0] = const_var_dim_ids[i]; const_dim_ids[1] = dlDim; const_dim_ids[2] = dyDim; const_dim_ids[3] = dxDim; const_dim_ids[4] = nensDim;}
           if (this->const_vars->fields_arr[i].extdof == 0)
           {const_dim_ids[0] = const_var_dim_ids[i]; const_dim_ids[1] = diDim; const_dim_ids[2] = dyDim; const_dim_ids[3] = dxDim; const_dim_ids[4] = nensDim;}
         }
         ncwrap( ncmpi_def_var( ncid , this->const_vars->fields_arr[i].name.c_str() , REAL_NC , 5 , const_dim_ids , &const_var_ids[i]  ) , __LINE__ );
       ...
       for (int i=0; i<this->prog_vars->fields_arr.size(); i++)
       {
         ...
         if (this->prog_vars->fields_arr[i].topology->primal) {
           ...
           {prog_dim_ids[0] = tVar; prog_dim_ids[1] = prog_var_dim_ids[i]; prog_dim_ids[2] = plDim; prog_dim_ids[3] = pyDim; prog_dim_ids[4] = pxDim; prog_dim_ids[5] = nensDim;}
           if (this->prog_vars->fields_arr[i].extdof == 0)
           {prog_dim_ids[0] = tVar; prog_dim_ids[1] = prog_var_dim_ids[i]; prog_dim_ids[2] = piDim; prog_dim_ids[3] = pyDim; prog_dim_ids[4] = pxDim; prog_dim_ids[5] = nensDim;}
         }
         else {
           if (this->prog_vars->fields_arr[i].extdof == 1)
           {prog_dim_ids[0] = tVar; prog_dim_ids[1] = prog_var_dim_ids[i]; prog_dim_ids[2] = dlDim; prog_dim_ids[3] = dyDim; prog_dim_ids[4] = dxDim; prog_dim_ids[5] = nensDim;}
           if (this->prog_vars->fields_arr[i].extdof == 0)
           {prog_dim_ids[0] = tVar; prog_dim_ids[1] = prog_var_dim_ids[i]; prog_dim_ids[2] = diDim; prog_dim_ids[3] = dyDim; prog_dim_ids[4] = dxDim; prog_dim_ids[5] = nensDim;}
         }
         ncwrap( ncmpi_def_var( ncid , this->prog_vars->fields_arr[i].name.c_str() , REAL_NC , 6 , prog_dim_ids , &prog_var_ids[i]  ) , __LINE__ );
       ...
       for (int i=0; i<this->diag_vars->fields_arr.size(); i++)
       {
         ...
         if (this->diag_vars->fields_arr[i].topology->primal) {
           ...
           {diag_dim_ids[0] = tVar; diag_dim_ids[1] = diag_var_dim_ids[i]; diag_dim_ids[2] = plDim; diag_dim_ids[3] = pyDim; diag_dim_ids[4] = pxDim; diag_dim_ids[5] = nensDim;}
           if (this->diag_vars->fields_arr[i].extdof == 0)
           {diag_dim_ids[0] = tVar; diag_dim_ids[1] = diag_var_dim_ids[i]; diag_dim_ids[2] = piDim; diag_dim_ids[3] = pyDim; diag_dim_ids[4] = pxDim; diag_dim_ids[5] = nensDim;}
         }
         else { 
           if (this->diag_vars->fields_arr[i].extdof == 1)
           {diag_dim_ids[0] = tVar; diag_dim_ids[1] = diag_var_dim_ids[i]; diag_dim_ids[2] = dlDim; diag_dim_ids[3] = dyDim; diag_dim_ids[4] = dxDim; diag_dim_ids[5] = nensDim;}
           if (this->diag_vars->fields_arr[i].extdof == 0)
           {diag_dim_ids[0] = tVar; diag_dim_ids[1] = diag_var_dim_ids[i]; diag_dim_ids[2] = diDim; diag_dim_ids[3] = dyDim; diag_dim_ids[4] = dxDim; diag_dim_ids[5] = nensDim;}
         }
         ncwrap( ncmpi_def_var( ncid , this->diag_vars->fields_arr[i].name.c_str() , REAL_NC , 6 , diag_dim_ids , &diag_var_ids[i]  ) , __LINE__ );
      ...
      statDimIds[0] = stat_var_dim_ids[l];
       ncwrap( ncmpi_def_var( ncid , this->statistics->stats_arr[l].name.c_str() , REAL_NC , 3 , statDimIds , &stat_ids[l]  ) , __LINE__ );
     }}
  ...
  template<uint nprog, uint nconst, uint ndiag, uint nstats> void FileIO<nprog,nconst,ndiag,nstats>::output(int nstep, real time)
  {
    ...
    ncwrap( ncmpi_open( MPI_COMM_WORLD , this->outputName.c_str() , NC_WRITE , MPI_INFO_NULL , &ncid ) , __LINE__ );
      ...
      for (int l=0; l<this->prog_vars->fields_arr.size(); l++)
      {
        ...
        prog_start[0] = this->numOut; prog_start[1] = 0; prog_start[2] = 0; prog_start[3] = this->prog_vars->fields_arr[l].topology->j_beg; prog_start[4] = this->prog_vars->fields_arr[l].topology->i_beg; prog_start[5] = 0;
        prog_count[0] = 1; prog_count[1] = this->prog_vars->fields_arr[l].total_dofs; prog_count[2] = this->prog_vars->fields_arr[l]._nz; prog_count[3] = this->prog_vars->fields_arr[l].topology->n_cells_y; prog_count[4] = this->prog_vars->fields_arr[l].topology->n_cells_x; prog_count[4] = this->prog_vars->fields_arr[l].topology->nens;
      ...
      for (int l=0; l<this->diag_vars->fields_arr.size(); l++)
      {
        ...
        diag_start[0] = this->numOut; diag_start[1] = 0; diag_start[2] = 0; diag_start[3] = this->diag_vars->fields_arr[l].topology->j_beg; diag_start[4] = this->diag_vars->fields_arr[l].topology->i_beg; diag_start[5] = 0;
        diag_count[0] = 1; diag_count[1] = this->diag_vars->fields_arr[l].total_dofs; diag_count[2] = this->diag_vars->fields_arr[l]._nz; diag_count[3] = this->diag_vars->fields_arr[l].topology->n_cells_y; diag_count[4] = this->diag_vars->fields_arr[l].topology->n_cells_x; diag_count[5] = this->diag_vars->fields_arr[l].topology->nens;
  ...
  template<uint nprog, uint nconst, uint ndiag, uint nstats> void FileIO<nprog,nconst,ndiag,nstats>::outputInit(real time)
  {
    ...
    for (int l=0; l<this->const_vars->fields_arr.size(); l++)
    {
      ...
      const_start[0] = 0; const_start[1] = 0; const_start[2] = this->const_vars->fields_arr[l].topology->j_beg; const_start[3] = this->const_vars->fields_arr[l].topology->i_beg; const_start[4] = 0;
      const_count[0] = this->const_vars->fields_arr[l].total_dofs; const_count[1] = this->const_vars->fields_arr[l]._nz; const_count[2] = this->const_vars->fields_arr[l].topology->n_cells_y; const_count[3] = this->const_vars->fields_arr[l].topology->n_cells_x; const_count[4] = this->const_vars->fields_arr[l].topology->nens;
    ...
    for (int l=0; l<this->prog_vars->fields_arr.size(); l++)
    {
      ...
      prog_start[0] = this->numOut; prog_start[1] = 0; prog_start[2] = 0; prog_start[3] = this->prog_vars->fields_arr[l].topology->j_beg; prog_start[4] = this->prog_vars->fields_arr[l].topology->i_beg; prog_start[5] = 0;
      prog_count[0] = 1; prog_count[1] = this->prog_vars->fields_arr[l].total_dofs; prog_count[2] = this->prog_vars->fields_arr[l]._nz; prog_count[3] = this->prog_vars->fields_arr[l].topology->n_cells_y; prog_count[4] = this->prog_vars->fields_arr[l].topology->n_cells_x; prog_count[5] = this->prog_vars->fields_arr[l].topology->nens;
      ncwrap( PNETCDF_PUT_VAR_ALL( ncid , prog_var_ids[l] , prog_start , prog_count , this->prog_temp_arr[l].createHostCopy().data() ) , __LINE__ );
    }}
...
// ADD THIS
  ...
  template<uint nprog, uint nconst, uint ndiag, uint nstats>  void FileIO<nprog,nconst,ndiag,nstats>::outputStats(const Stats<nprog, nconst, nstats> &stats)
  {
    ...
    statStart[0] = 0; statStart[1] = 0; statStart[2] = 0; statCount[1] = this->statistics->statsize; statCount[2] = this->statistics->nens;
    for (int l=0; l<this->statistics->stats_arr.size(); l++)
    {
      if (this->statistics->stats_arr[l].ndofs >0) {
