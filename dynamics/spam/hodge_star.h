...
template<uint ndofs, uint ord, uint off=ord/2 -1> void YAKL_INLINE compute_H(SArray<real,1,ndims> &u, const real5d vvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
...
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_H(real5d uvar, const real5d vvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndims> u;
  compute_H<ndofs, ord, off> (u, vvar, pgeom, dgeom, is, js, ks, i, j, k, n);
  if (addmode == ADD_MODE::REPLACE) {for (int d=0; d<ndims; d++) { uvar(d, k+ks, j+js, i+is, n) = u(d);}}
  if (addmode == ADD_MODE::ADD) {for (int d=0; d<ndims; d++) { uvar(d, k+ks, j+js, i+is, n) += u(d);}}
}
...
real YAKL_INLINE compute_Hv(const real5d wvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  // THIS IS 2ND ORDER AT BEST...
  return wvar(0, k+ks-1, j+js, i+is, n) * dgeom.get_area_10entity(k+ks, j+js, i+is) / pgeom.get_area_01entity(k+ks-1, j+js, i+is);
}
...
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_Hv(real5d uwvar, const real5d wvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  real uw = compute_Hv(wvar, pgeom, dgeom, is, js, ks, i, j, k, n);
  if (addmode == ADD_MODE::REPLACE) { uwvar(0, k+ks, j+js, i+is, n) = uw;}
  if (addmode == ADD_MODE::ADD) { uwvar(0, k+ks, j+js, i+is, n) += uw;}
}
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_Hv(SArray<real,1,1> &uwvar, const real5d wvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  real uw = compute_Hv(wvar, pgeom, dgeom, is, js, ks, i, j, k, n);
  ...
  template<uint ndofs, uint ord, uint off=ord/2 -1> void YAKL_INLINE compute_Hext(SArray<real,1,ndims> &u, const real5d vvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
  {
    SArray<real,2,ndims,ord-1> v;
    SArray<real,2,ndims,ord-1> Hgeom;
    for (int p=0; p<ord-1; p++) {
...
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_Hext(real5d uvar, const real5d vvar, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndims> u;
...
template<uint ndofs, uint ord, uint off=ord/2 -1> void YAKL_INLINE compute_I(SArray<real,1,ndofs> &x0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
...
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_I(real5d var0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndofs> x0;
  compute_I<ndofs, ord, off> (x0, var, pgeom, dgeom, is, js, ks, i, j, k, n);
  if (addmode == ADD_MODE::REPLACE) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) = x0(l);}}
  if (addmode == ADD_MODE::ADD) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) += x0(l);}}
...
template<uint ndofs, uint hord,  uint vord, uint hoff=hord/2 -1, uint voff=vord/2 -1> void YAKL_INLINE compute_Iext(SArray<real,1,ndofs> &x0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
...
template<uint ndofs, uint hord,  uint vord, ADD_MODE addmode=ADD_MODE::REPLACE, uint hoff=hord/2 -1, uint voff=vord/2 -1> void YAKL_INLINE compute_Iext(real5d var0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndofs> x0;
  compute_Iext<ndofs, hord, vord, hoff, voff> (x0, var, pgeom, dgeom, is, js, ks, i, j, k, n);
  if (addmode == ADD_MODE::REPLACE) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) = x0(l);}}
  if (addmode == ADD_MODE::ADD) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) += x0(l);}}
}
...
template<uint ndofs, uint ord, uint off=ord/2 -1> void YAKL_INLINE compute_J(SArray<real,1,ndofs> &x0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
SArray<real,3,ndofs,ndims,ord-1> x;
...
template<uint ndofs, uint ord, ADD_MODE addmode=ADD_MODE::REPLACE, uint off=ord/2 -1> void YAKL_INLINE compute_J(real5d var0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndofs> x0;
...
template<uint ndofs, uint hord, uint vord, uint hoff=hord/2 -1, uint voff=vord/2 -1> void YAKL_INLINE compute_Jext(SArray<real,1,ndofs> &x0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
SArray<real,3,ndofs,ndims,hord-1> x;
...
template<uint ndofs, uint hord,  uint vord, ADD_MODE addmode=ADD_MODE::REPLACE, uint hoff=hord/2 -1, uint voff=vord/2 -1> void YAKL_INLINE compute_Jext(real5d var0, const real5d var, Geometry &pgeom, Geometry &dgeom, int is, int js, int ks, int i, int j, int k, int n)
{
  SArray<real,1,ndofs> x0;
  compute_Jext<ndofs,hord,vord,hoff,voff> (x0, var, pgeom, dgeom, is, js, ks, i, j, k, n);
  if (addmode == ADD_MODE::REPLACE) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) = x0(l);}}
  if (addmode == ADD_MODE::ADD) {for (int l=0; l<ndofs; l++) {var0(l, k+ks, j+js, i+is, n) += x0(l);}}
}
