...
public:
  ...
  void init(PamCoupler &coupler) {
    ...
    initialize_variables(primal_topology, dual_topology, prog_dofs_arr,
                         const_dofs_arr, aux_dofs_arr, diag_dofs_arr,
                         prog_names_arr, const_names_arr, aux_names_arr,
                         diag_names_arr, prog_topo_arr, const_topo_arr,
                         aux_topo_arr, diag_topo_arr);

    prognostic_vars.initialize("x", prog_names_arr, prog_topo_arr,
                               prog_dofs_arr);
    constant_vars.initialize("cons", const_names_arr, const_topo_arr,
                             const_dofs_arr);
    diagnostic_vars.initialize("diag", diag_names_arr, diag_topo_arr,
                               diag_dofs_arr);
    ...
    io.initialize(params.outputName, primal_topology, dual_topology, par,
                  prognostic_vars, constant_vars, diagnostic_vars, stats);
    ...
    tendencies.initialize(coupler, params, primal_topology, dual_topology,
                          ...
                          const_exchange);
    diagnostics.initialize(primal_topology, dual_topology, primal_geometry,
                           dual_geometry, diag_exchange);
    ...
    for (uint nstep = 0; nstep < params.crm_per_phys; nstep++) {
      ...
      if ((nstep + prevstep) % params.Nout == 0) {
        serial_print("dycore step " + std::to_string((nstep + prevstep)) +
                     ...
                     par.masterproc);
        diagnostics.compute_diag(constant_vars, prognostic_vars,
                                 diagnostic_vars);
        io.output(etime);
