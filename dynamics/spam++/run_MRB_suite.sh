  ...
  do
    ...
    do
      ...
      do
        ...
        ./build/cmake_linuxlaptop.sh mce2Dext build_consts.build &> build.out
        make &>> build.out
        #mpirun.mpich -n 4 ./layermodel2D runsuites/MRB/MRB${SIZE}.input &> run.out
        mpirun.mpich -n 4 ./extrudedmodel2D runsuites/MRB/MRB${SIZE}.input &> run.out
        mv *.png *.nc *.build *.out MRB/MRB${SIZE}-${RECONTYPE}${RECONORDER}-HODGE${DIFFORDER}
        cp runsuites/MRB/MRB${SIZE}.input MRB/MRB${SIZE}-${RECONTYPE}${RECONORDER}-HODGE${DIFFORDER}
      done
    done  
  done
done
