module grid
  ...
  real(crm_rknd), bind(C) :: z    (ncrms,nz)       ! height of the pressure levels above surface,m
  real(crm_rknd), bind(C) :: pres (ncrms,nzm)  ! pressure,mb at scalar levels
  real(crm_rknd), bind(C) :: zi   (ncrms,nz)      ! height of the interface levels
  real(crm_rknd), bind(C) :: presi(ncrms,nz)   ! pressure,mb at interface levels
  real(crm_rknd), bind(C) :: adz  (ncrms,nzm)   ! ratio of the thickness of scalar levels to dz
  real(crm_rknd), bind(C) :: adzw (ncrms,nz)   ! ratio of the thinckness of w levels to dz
  real(crm_rknd), bind(C) :: dt3  (3)         ! dynamical timesteps for three most recent time steps
  real(crm_rknd), bind(C) :: dz   (ncrms)      ! constant grid spacing in z direction (when dz_constant=.true.)
