...
contains
  ...
  subroutine periodic(flag)
    ...
    if(flag.eq.2) then
      ...
      do i = 1,nsgs_fields
        if(dosgs.and.advect_sgs) call bound_exchange(sgs_field(:,:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,3,3,3,3,4+i)
      end do
      do i = 1,nmicro_fields
        if(   i.eq.index_water_vapor             &
        .or. docloud.and.flag_precip(i).ne.1    &
        .or. doprecip.and.flag_precip(i).eq.1 ) then
          call bound_exchange(micro_field(:,:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,3,3,3,3,4+nsgs_fields+nsgs_fields_diag+i)
        endif
      end do
    endif
    ...
    if(flag.eq.3) then
      ...
      do i = 1,nmicro_fields
        ...
        .or. docloud.and.flag_precip(i).ne.1    &
        .or. doprecip.and.flag_precip(i).eq.1 ) then
          call bound_exchange(micro_field(:,:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm,1,1,1,1,4+nsgs_fields+nsgs_fields_diag+i)
        endif
      end do
    endif
    ...
    if(flag.eq.4) then
      do i = 1,nsgs_fields_diag
        ...
        call bound_exchange(sgs_field_diag(:,:,:,:,i),dimx1_d,dimx2_d,dimy1_d,dimy2_d,nzm,1+dimx1_d,dimx2_d-nx,YES3D+dimy1_d,1-YES3D+dimy2_d-ny,4+nsgs_fields+i)
      end do
    end if
